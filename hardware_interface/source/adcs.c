...
ADCS_returnState HAL_ADCS_get_boot_program_stat(ADCS_boot_program_stat* boot_program_stat) {
    return ADCS_get_boot_program_stat(&boot_program_stat->mcu_reset_cause, &boot_program_stat->boot_cause, &boot_program_stat->boot_count, &boot_program_stat->boot_idx);
}
...
ADCS_returnState HAL_ADCS_get_last_logged_event(ADCS_last_logged_event* last_logged_event) {
    return ADCS_get_last_logged_event(&last_logged_event->time, &last_logged_event->event_id, &last_logged_event->event_param);
}
...
ADCS_returnState HAL_ADCS_get_file_download_block_stat(ADCS_file_download_block_stat* file_download_block_stat) {
    return ADCS_get_file_download_block_stat(&file_download_block_stat->ready, &file_download_block_stat->param_err, &file_download_block_stat->crc16_checksum, &file_download_block_stat->length);
}

ADCS_returnState HAL_ADCS_get_file_info(ADCS_file_info* file_info) {
    return ADCS_get_file_info(&file_info->type, &file_info->updating, &file_info->counter, &file_info->size, &file_info->time, &file_info->crc16_checksum);
}
...
ADCS_returnState HAL_ADCS_get_EDAC_err_count(ADCS_EDAC_err_count* EDAC_err_count) {
    return ADCS_get_EDAC_err_count(&EDAC_err_count->single_sram, &EDAC_err_count->double_sram, &EDAC_err_count->multi_sram);
}
...
ADCS_returnState HAL_ADCS_get_program_info(ADCS_program_info* program_info) {
    return ADCS_get_program_info(&program_info->index, &program_info->busy, &program_info->file_size, &program_info->crc16_checksum);
}
...
ADCS_returnState HAL_ADCS_get_jpg_cnv_progress(ADCS_jpg_cnv_progress* jpg_cnv_progress) {
    return ADCS_get_jpg_cnv_progress(&jpg_cnv_progress->percentage, &jpg_cnv_progress->result, &jpg_cnv_progress->file_counter);
}
...
ADCS_returnState HAL_ADCS_get_execution_times(ADCS_execution_times* execution_times) {
    return ADCS_get_execution_times(&execution_times->adcs_update, &execution_times->sensor_comms, &execution_times->sgp4_propag, &execution_times->igrf_model);
}
