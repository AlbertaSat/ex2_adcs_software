 ...
 * 		Success of function defined in adcs_types.h
 */
ADCS_returnState ADCS_run_selected_program (void){ //* could be merged with the previous one in services
 ...
 * 		Success of function defined in adcs_types.h
 */
ADCS_returnState ADCS_read_program_info (uint8_t index){ //* why would we need the enum
	uint8_t command[2];
 ...
 * @param index
 * 		Table 66 - Source Program index				//* not good for services
 * 		0 : BootLoader, 1 : Internal flash program, 2 : EEPROM,
 * 		3-9: External flash program 1-7,
 * 		10-17: SD user file 1-8
 * @attention
 ...
 * 		Success of function defined in adcs_types.h
 */
ADCS_returnState ADCS_copy_program_internal_flash (uint8_t index, uint8_t overwrite_flag){ //* why would we need the enum
	uint8_t command[3];
...
* 		sram1, sram2, sram_latch_not_recovered, sram_latch_recovered, sd_initial_err, sd_read_err,
* 		sd_write_err, external_flash_err, internal_flash_err, eeprom_err, bad_boot_reg, comms_radio_err
* @attention
