...
void test_ADCS_common_commands(void)
{
    ...
    TEST_ASSERT_EQUAL_INT(ADCS_OK, ADCS_load_file_download_block(ftype, counter,
offset, blength));
    ...
    command[3] = blength;
    send_uart_telecommand_ExpectAndReturn(command, 4, ADCS_OK);
    TEST_ASSERT_EQUAL_INT(ADCS_OK, ADCS_finalize_upload_block(fdest, offset,
blength));
    ...
    command[2] = ign_hole;
    send_uart_telecommand_ExpectAndReturn(command, 3, ADCS_OK);
    TEST_ASSERT_EQUAL_INT(ADCS_OK, ADCS_initiate_download_burst(msg_l,
ign_hole));
    //ADCS_invalid_TC_ID
